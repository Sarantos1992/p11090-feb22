...
def play_game():            
    ...
    while lids:
        ...
        lid = lids.pop()
        tmp_pos = randrange(9)
        if check_if_can_fill(tmp_pos,int(lid)):            
            fill(tmp_pos,int(lid))            
            if check_for_win():
                print("Winner at step " + str(steps))
                print_board()
                break                
        else:
            lids.insert(0,lid)          # sos not append
...
small_lids = [1] * 9      # SMALL --> 1 
medium_lids = [2] * 9    # MEDIUM --> 2
big_lids = [3] * 9          # BIG --> 3
...
for i in range(100):
    ...
    lids = []
    lids = small_lids + medium_lids + big_lids
    shuffle(lids)        
